...
|---|---|---|---|---|
| `X14` | main E-stop input (per `.vb` source comment) | `MainModule.vb:1956` | **Owner-confirmed functional 2026-08-18:** owner states `X14` does function as the emergency stop in practice, describes the physical device as a large red pushbutton (visually consistent with the ISO 13850 mushroom-head red-button convention), and confirms the behavior is a complete shutdown of the whole device, manually triggered by the operator. **Circuit-level answers obtained 2026-08-18, owner-reported:** (1) contact is **open (NO) in the released/idle state**; (2) **not** dual-channel — single channel only; (3) **no** wire-break/line monitoring; (4) reset is **not automatic** — manual action only. | 🔴 **SAFETY FLAG — requires qualified electrical/safety review before any reliance on this input, not just further chat confirmation.** A normally-open-in-idle, single-channel, unmonitored circuit is the opposite of the conventional fail-safe emergency-stop wiring pattern (which is normally-closed while idle, so a cut wire, loose connector, or power loss self-reveals as a stop condition). As reported, a wire break, connector failure, or loss of 24V to this input would look electrically identical to "not pressed" — the system could fail to register a real stop condition instead of failing toward safety. **This document does not conclude the circuit is unsafe** — it may use a different, still-valid safety strategy (e.g., a safety relay/PLC that treats loss-of-signal as fault through other means), or the owner's description of "open when released" may refer to something other than the safety contact itself. Either way, this cannot be resolved by further verbal description; it requires a qualified person tracing the actual physical circuit against ISO 13849-1's category/PL requirements before `X14` is used for production arming. Do not treat any owner-reported answer in this row as a substitute for that review. UNKNOWN polarity/topology/monitoring beyond what is stated here; cannot satisfy safety readiness |
